...
async def main():
    ...
    def help_card():
        return (
            ...
            "—— *任务管理* ——\n"
            "／listtasks  列出任务\n"
            "／addtask 目标 | CRON/间隔 | 命令(多条用||) | 账号别名 | 备注 | 消息延迟(-=无延迟) | 发言ID(-=本账号/none=禁用)\n"
            "／edittask ID | 目标 | CRON/间隔 | 命令(多条用||) | 账号别名 | 备注 | 消息延迟(-=无) | 发言ID(-=本账号/none=禁用)\n"
            "／deltask ID 删除任务\n"
            "／toggle ID 启/停任务\n"
            ...
            "—— *时间调整* ——\n"
            "／nextinterval ID 查看某个间隔任务剩余时间；\n"
            "／nextinterval all 查看全部间隔任务；\n"
            "／delaynext ID | 时间间隔 临时调整间隔任务的下一次执行时间；\n\n"
            "—— *任务模板* ——\n"
            "／listtpl 查看模板列表\n"
            "／addtpl 名称 | 目标 | 命令(多条用||)\n"
            "／edittpl ID | 名称 | 目标 | 命令(多条用||)\n"
            "编辑时字段用 _ 可不修改\n"
            "／addtpltask 模板ID | CRON/间隔 | 账号(多条用,) | 备注 | 消息延迟(-=无) | 发言ID(-/none/多条用,)\n"
            "模板任务 ID 以 T 开头，例如 T1；\n"
            "可使用 ／edittask T1 | 模板ID | CRON/间隔 | 账号 | 备注 | 消息延迟 | 发言ID 对模板任务进行修改\n\n"
            "—— *账号管理* ——\n"
            "／adduser 别名 | 手机号(含国家码)\n"
            "／code 别名 | 验证码\n"
            "／pass 别名 | 二步验证密码（如需要）\n"
